...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, modelPath, filePath, render_mode = None):
        ...
        self.enable_mass_changes = True
        self.enable_torque_changes = True
        self.ablation_modes = [
            "full",
            "no_imitation",
            "no_height",
        ...
        self.current_ablation_mode = "full"  # Default to full reward
        self.last_ablation_update_episode = 0

        self.base_actuator_gears = self.model.actuator_gear.copy()
        self.modified_actuator_gears = self.base_actuator_gears.copy()
        self.last_torque_update_episode = 0
        ...
        self.last_mass_update_episode = 0
    ...
    def check_floor_contact(self):
        ...
        for body_name in self.body_names_to_check:
            ...
            if body_name in ['right_ankle', 'left_ankle']:
                if len(contact_points) >= 2:
                    distances = [np.linalg.norm(contact_points[i] - contact_points[j]) for i in range(len(contact_points)) for j in range(i+1, len(contact_points))]
                    max_distance = max(distances) if distances else 0.0
                    if max_distance > 0.05:  # Ensure points span the foot (~0.1m length)
                        foot_quat = self.get_name_quat(body_name)
                        z_axis = self.quat_to_z_axis(foot_quat)
                        if np.dot(z_axis, np.array([0, 0, 1])) > 0.95:  # Flat within 5 degrees
    ...
    def reward(self):
        ...
        pose_mse = np.mean(pose_diff**2)

        # Velocity MSE
        vel_mse = np.mean((cur_bangvel[3:] - e_bangvel[3:])**2)

        # End-effector MSE
        ee_mse = np.mean((cur_ee - e_ee)**2)

        # COM MSE
        com_mse = np.mean((cur_com - e_com)**2)
        self.imitation_mse = w_p * pose_mse + w_v * vel_mse + w_e * ee_mse + w_c * com_mse
        ...
        # Apply ablation
        if self.current_ablation_mode == 'no_imitation':
            w_S = 0.0
        elif self.current_ablation_mode == 'no_height':
            w_G = 0.0
        elif self.current_ablation_mode == 'no_stability':
            w_stab = 0.0
        elif self.current_ablation_mode == 'no_distance':
            w_dist = 0.0
        elif self.current_ablation_mode == 'no_squat':
            w_squat = 0.0
        elif self.current_ablation_mode == 'no_leg':
            w_leg = 0.0
        elif self.current_ablation_mode == 'no_sym':
            w_sym = 0.0
        elif self.current_ablation_mode == 'no_takeoff':
            w_takeoff = 0.0
        elif self.current_ablation_mode == 'no_yaw':
            w_yaw = 0.0
        elif self.current_ablation_mode == 'no_upright':
            w_upright = 0.0
    ...
    def step(self, action):
        ...
        info = {
            ...
            'jump_distance': self.get_com()[0],
            'com_height': self.get_com()[2],
            'symmetry_score': self.symmetry,
            'imitation_mse': self.imitation_mse
        }
    ...
    def reset_model(self):
        ...
        if self.episode % 5000 == 0 and self.episode != self.last_mass_update_episode and self.enable_mass_changes:
            self.last_mass_update_episode = self.episode
            for body_name in self.modified_body_masses:
                scale = np.random.uniform(0.8, 1.2)
                self.modified_body_masses[body_name] = self.base_body_masses[body_name] * scale
                body_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, body_name)
                self.model.body_mass[body_id] = self.modified_body_masses[body_name]
        
        if self.episode % 5000 == 0 and self.episode != self.last_torque_update_episode and self.enable_torque_changes:
            self.last_torque_update_episode = self.episode
            for i in range(self.model.nu):
                scale = np.random.uniform(0.8, 1.2)
                self.modified_actuator_gears[i] = self.base_actuator_gears[i] * scale
                self.model.actuator_gear[i] = self.modified_actuator_gears[i]
        
        if self.episode % 5000 == 0 and self.episode != self.last_ablation_update_episode and self.current_ablation_mode == "full":
            self.last_ablation_update_episode = self.episode
            self.current_ablation_mode = np.random.choice(self.ablation_modes)
