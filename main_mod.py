...
import datetime
import argparse
from environment.humanoidEnvTry import HumanoidEnv
...
from utils.ppoUtils import evaluate_data, evaluate_rest, evaluate_foot_prog
...
def create_motion_trail(env, agent, max_action, turns=1, save_every=20, step_x=2000):
    ...
    for j in range(turns):
        ...
        canvas.save(f"jump_motion_progression_{j}.png")
        print(f"Saved jump_motion_progression_{j}.png with {num_frames} frames")

    return r

trainingModes = ["jumpInit_Mod"]
# "jumpInit_og", 
def train_loop(config):
    ...
    if config.render:
        ...
        ep_r = evaluate_foot_prog(env, agent, max_action=config.max_action, turns=150)
        ...
        if config.best == True:
            ...
            print(config.model_index, trainingModes[index])
            agent.load(f"{trainingModes[index]}", config.model_index)
            normalizer_state = np.load(f"./model/norm/{trainingModes[index]}_normalizer_{config.model_index}.npy", allow_pickle=True).item()
            agent.obs_normalizer.rms.load_variables(normalizer_state)
        env = HumanoidEnv(modelPath, filePath, render_mode="rgb_array")
        env.set_mode = trainingModes[index]
        ep_r = create_motion_trail(env, agent, max_action=config.max_action, turns=1)
